...
#include "ITwinCesiumCreditSystemBPLoader.h"
#include "ITwinCesiumRuntime.h"
#include "CesiumUtility/CreditSystem.h"
#include "Engine/World.h"
#include "EngineUtils.h"
#include "ITwinScreenCreditsWidget.h"
#include <string>
...
/*static*/ UObject* AITwinCesiumCreditSystem::CesiumCreditSystemBP = nullptr;
namespace {

/**
 * @brief Tries to find the default credit system in the given level.
 *
 * This will search all actors of the given level for a `AITwinCesiumCreditSystem`
...
AITwinCesiumCreditSystem* findValidDefaultCreditSystem(ULevel* Level) {
  ...
  AActor** DefaultCreditSystemPtr =
      Actors.FindByPredicate([](AActor* const& InItem) {
        ...
        if (!InItem->IsA(AITwinCesiumCreditSystem::StaticClass())) {
          return false;
        }
  ...
  return Cast<AITwinCesiumCreditSystem>(DefaultCreditSystem);
}
...
FName AITwinCesiumCreditSystem::DEFAULT_CREDITSYSTEM_TAG =
    FName("ITWIN_DEFAULT_CREDITSYSTEM");

/*static*/ AITwinCesiumCreditSystem*
AITwinCesiumCreditSystem::GetDefaultCreditSystem(const UObject* WorldContextObject) {
  ...
  AITwinCesiumCreditSystem* pCreditSystem = nullptr;
  ...
  for (TActorIterator<AActor> actorIterator(
           world,
           AITwinCesiumCreditSystem::StaticClass(),
           flags);
       actorIterator;
       ++actorIterator) {
    ...
    if (actor->GetLevel() == world->PersistentLevel &&
      ...
      pCreditSystem = Cast<AITwinCesiumCreditSystem>(actor);
      break;
    }
  }
  if (!pCreditSystem) {
    ...
    // existing projects
    AITwinCesiumCreditSystem* pCreditSystemCandidate =
  ...
  if (!pCreditSystem) {
    ...
    pCreditSystem = world->SpawnActor<AITwinCesiumCreditSystem>(
...
AITwinCesiumCreditSystem::AITwinCesiumCreditSystem()
    : AActor(),
...
void AITwinCesiumCreditSystem::BeginPlay() {
  Super::BeginPlay();
...
void AITwinCesiumCreditSystem::EndPlay(const EEndPlayReason::Type EndPlayReason) {
...
void AITwinCesiumCreditSystem::OnConstruction(const FTransform& Transform) {
...
void AITwinCesiumCreditSystem::updateCreditsViewport(bool recreateWidget) {
  ...
  if (!IsValid(CreditsWidget) || recreateWidget) {
    CreditsWidget =
        CreateWidget<UITwinScreenCreditsWidget>(GetWorld(), CreditsWidgetClass);
  }
...
void AITwinCesiumCreditSystem::removeCreditsFromViewports() {
#if WITH_EDITOR
...
#if WITH_EDITOR
void AITwinCesiumCreditSystem::OnRedrawLevelEditingViewports(bool) {
  this->updateCreditsViewport(false);
}

void AITwinCesiumCreditSystem::OnPreBeginPIE(bool bIsSimulating) {
...
void AITwinCesiumCreditSystem::OnEndPIE() { this->updateCreditsViewport(false); }

void AITwinCesiumCreditSystem::OnCleanseEditor() {
  // This is called late in the process of unloading a level.
  this->removeCreditsFromViewports();
}
#endif

bool AITwinCesiumCreditSystem::ShouldTickIfViewportsOnly() const { return true; }

void AITwinCesiumCreditSystem::Tick(float DeltaTime) {
...
FString AITwinCesiumCreditSystem::ConvertHtmlToRtf(std::string html) {
  TidyDoc tdoc;
